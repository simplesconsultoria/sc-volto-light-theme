...
- Brought the design system documentation up to date with the Storybook work: the story count and classification, the container and theme decorator conventions, the blocks gallery result, and where story images belong. @ericof [#22](https://github.com/simplesconsultoria/sc-volto-light-theme/pull/22)
- Rewrote the design system manifest from verified evidence and moved it under `docs/design-system/`, leaving a short pointer at the repository root. The previous draft carried 69 unverified claims and 51 open items, and re-documented the schemas of blocks belonging to dependencies; the manifest now covers what the add-on owns, states the rules that govern how it composes with the layers beneath it, and points at the runtime inventory rather than repeating a list that goes stale. @ericof [#22](https://github.com/simplesconsultoria/sc-volto-light-theme/pull/22)
- Added preview pages for the four blocks the add-on ships, generated from the built Storybook rather than hand-written: each is the real component output with its computed styles inlined, captured in both colour modes and, for the blocks that read `--theme-*`, in both block themes. @ericof [#22](https://github.com/simplesconsultoria/sc-volto-light-theme/pull/22)
- Reduced the gaps document to what is still open. It had grown to hold a record of every defect fixed along the way, which obscured the point of the document; the durable findings about registered custom properties and stylesheet scoping moved into the manifest, where they belong as rules rather than as history, and the sections were renumbered with the inbound references updated to match. @ericof [#22](https://github.com/simplesconsultoria/sc-volto-light-theme/pull/22)
- Reconciled the design-system documentation with the palette and theme-schema changes: the contrast section now records both WCAG failures as fixed and what fixed them, the per-project overlay section describes the 38 `_light`/`_dark` colour fields the control panel actually serves, and the content-type colours, base ramp and type scale carry their current values. `GAPS.md` was renumbered when its resolved sections were removed, so every cross-reference in `DESIGN_SYSTEM.md`, `INVENTORY.md` and the preview pages was re-pointed; the identical `narrow` and `default` container widths were restored as an open gap, having been removed while still true. @ericof [#22](https://github.com/simplesconsultoria/sc-volto-light-theme/pull/22)
- Added a machine-extracted design system reference under `docs/design-system/`: an inventory of every block, listing variation, theme mapping and custom property with its resolved light and dark value; a gap analysis carrying a full WCAG contrast audit of the shipped palette; and self-contained preview pages for the colour system, the block themes and the type scale, marked up for import into Claude Design. @ericof [#22](https://github.com/simplesconsultoria/sc-volto-light-theme/pull/22)
